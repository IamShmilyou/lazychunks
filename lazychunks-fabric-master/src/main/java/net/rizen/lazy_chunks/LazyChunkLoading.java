...
public class LazyChunkLoading {
    ...
    private static final int MAX_ONE_PERCENT_LOW_SAMPLE_COUNT = 3;
    private static final double ONE_PERCENT_LOW_OUTLIER_CAP_MULTIPLIER = 2.5;
    private static final double ONE_PERCENT_LOW_DROP_SMOOTHING_FACTOR = 0.35;
    private static final double ONE_PERCENT_LOW_RECOVERY_SMOOTHING_FACTOR = 0.15;
    ...
    private static final int MAX_TRACKED_FRAME_TIME_MICROS = 1_000_000;
    ...
    private static final long LOW_ONE_PERCENT_DURATION_RESET_INTERVAL_NANOS = 12L * NANOS_PER_SECOND;
    private static final long ADAPTIVE_CORE_DECREASE_INTERVAL_NANOS = 500_000_000L;
    ...
    private static long lastAdaptiveCoreDecreaseNanos = 0;
    ...
    private static void updateOnePercentLowFps(long currentTimeNanos) {
        ...
        synchronized (FRAME_TIME_HISTORY_LOCK) {
            ...
            measuredFps = calculateOnePercentLowFpsLocked();
        }
    ...
    private static double calculateOnePercentLowFpsLocked() {
        if (frameTimeHistoryCount < MIN_FRAME_TIME_SAMPLES) {
            return 0.0;
        }

        int count = Math.min(frameTimeHistoryCount, frameTimeHistoryMicros.length);
        int sampleStartIndex = frameTimeHistoryCount >= FRAME_TIME_HISTORY_SIZE ? frameTimeHistoryIndex : 0;
        int validSamples = 0;
        int slowestFrameTimeMicros = 0;
        int secondSlowestFrameTimeMicros = 0;
        int thirdSlowestFrameTimeMicros = 0;

        for (int i = 0; i < count; i++) {
            int frameTimeMicros = getFrameTimeSampleMicros(sampleStartIndex, i);
            if (frameTimeMicros > 0) {
                validSamples++;
                frameTimeMicros = capIsolatedFrameTimeMicros(frameTimeMicros, sampleStartIndex, i, count);

                if (frameTimeMicros >= slowestFrameTimeMicros) {
                    thirdSlowestFrameTimeMicros = secondSlowestFrameTimeMicros;
                    secondSlowestFrameTimeMicros = slowestFrameTimeMicros;
                    slowestFrameTimeMicros = frameTimeMicros;
                } else if (frameTimeMicros >= secondSlowestFrameTimeMicros) {
                    thirdSlowestFrameTimeMicros = secondSlowestFrameTimeMicros;
                    secondSlowestFrameTimeMicros = frameTimeMicros;
                } else if (frameTimeMicros > thirdSlowestFrameTimeMicros) {
                    thirdSlowestFrameTimeMicros = frameTimeMicros;
                }
            }
        }

        if (validSamples < MIN_FRAME_TIME_SAMPLES || slowestFrameTimeMicros <= 0) {
            return 0.0;
        }

        int slowFrameCount = Math.min(
                MAX_ONE_PERCENT_LOW_SAMPLE_COUNT,
                Math.max(1, (int) Math.ceil(validSamples * ONE_PERCENT_LOW_SAMPLE_RATIO))
        );
        long slowFrameTimeTotalMicros = slowFrameCount >= 2
                ? capExtremeSlowFrameTimeMicros(slowestFrameTimeMicros, secondSlowestFrameTimeMicros)
                : slowestFrameTimeMicros;

        if (slowFrameCount >= 2) {
            slowFrameTimeTotalMicros += secondSlowestFrameTimeMicros;
        }

        if (slowFrameCount >= 3) {
            slowFrameTimeTotalMicros += thirdSlowestFrameTimeMicros;
        }

        double averageSlowFrameTimeMicros = slowFrameTimeTotalMicros / (double) slowFrameCount;
        return MICROS_PER_SECOND / averageSlowFrameTimeMicros;
    }

    private static int getFrameTimeSampleMicros(int sampleStartIndex, int sampleOffset) {
        return frameTimeHistoryMicros[(sampleStartIndex + sampleOffset) % frameTimeHistoryMicros.length];
    }

    private static int capIsolatedFrameTimeMicros(
            int frameTimeMicros,
            int sampleStartIndex,
            int sampleOffset,
            int sampleCount
    ) {
        if (sampleOffset <= 0 || sampleOffset >= sampleCount - 1) {
            return frameTimeMicros;
        }

        int previousFrameTimeMicros = getFrameTimeSampleMicros(sampleStartIndex, sampleOffset - 1);
        int nextFrameTimeMicros = getFrameTimeSampleMicros(sampleStartIndex, sampleOffset + 1);
        int strongestNeighborFrameTimeMicros = Math.max(previousFrameTimeMicros, nextFrameTimeMicros);
        return capExtremeSlowFrameTimeMicros(frameTimeMicros, strongestNeighborFrameTimeMicros);
    }

    private static int capExtremeSlowFrameTimeMicros(int frameTimeMicros, int referenceFrameTimeMicros) {
        if (referenceFrameTimeMicros <= 0) {
            return frameTimeMicros;
        }

        int outlierCapMicros = (int) Math.round(referenceFrameTimeMicros * ONE_PERCENT_LOW_OUTLIER_CAP_MULTIPLIER);
        return Math.min(frameTimeMicros, outlierCapMicros);
    }
    ...
    private static void updateOnePercentLowFpsValue(double measuredFps) {
        ...
        double smoothingFactor = measuredFps < onePercentLowFps
                ? ONE_PERCENT_LOW_DROP_SMOOTHING_FACTOR
                : ONE_PERCENT_LOW_RECOVERY_SMOOTHING_FACTOR;
        onePercentLowFps += (measuredFps - onePercentLowFps) * smoothingFactor;

        if (onePercentLowFps <= 0) {
            onePercentLowFps = measuredFps;
        }
    }
    ...
    public static void recordFrameTime(double frameTimeMs) {
        ...
        synchronized (FRAME_TIME_HISTORY_LOCK) {
            ...
            frameTimeSampleCount = frameTimeHistoryCount;
        }
    ...
    private static int clampFrameTimeMicros(double frameTimeMs) {
        ...
        if (frameTimeMicros >= MAX_TRACKED_FRAME_TIME_MICROS) {
            return MAX_TRACKED_FRAME_TIME_MICROS;
        }
    ...
    private static void updateLowOnePercentDurationState(LazyChunksConfig config, long currentTimeNanos, int frameTimeSampleCount) {
        ...
        if (isBelowThreshold) {
            ...
            if (adaptiveBoostActivated) {
                long timeSinceActivation = currentTimeNanos - adaptiveBoostActivationTimeNanos;
                if (timeSinceActivation >= 8L * NANOS_PER_SECOND) {
                    resetAdaptiveBoostState();
                }
            } else {
                resetAdaptiveBoostState();
    ...
    private static void updateAdaptiveBoostState(LazyChunksConfig config, long currentTimeNanos) {
        ...
        updateAdaptiveExtraCores(targetExtraCores, currentTimeNanos);
    }
    ...
    private static void updateAdaptiveExtraCores(int targetExtraCores, long currentTimeNanos) {
        if (targetExtraCores >= adaptiveExtraCores) {
            adaptiveExtraCores = targetExtraCores;
            lastAdaptiveCoreDecreaseNanos = currentTimeNanos;
            return;
        }

        if (lastAdaptiveCoreDecreaseNanos == 0
                || currentTimeNanos - lastAdaptiveCoreDecreaseNanos >= ADAPTIVE_CORE_DECREASE_INTERVAL_NANOS) {
            adaptiveExtraCores--;
            lastAdaptiveCoreDecreaseNanos = currentTimeNanos;
        }
    }
    ...
    private static void resetAdaptiveBoostState() {
        ...
        lastAdaptiveCoreDecreaseNanos = 0;
