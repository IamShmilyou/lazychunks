...
public class LazyChunksConfig {
    ...
    private static final double MIN_ADAPTIVE_CORE_CHANGE_SECONDS = 0.5;
    private static final double MAX_ADAPTIVE_CORE_CHANGE_SECONDS = 5.0;
    ...
    /**
     * Minimum delay between adaptive core count changes.
     * Higher values make chunk budget changes smoother but slower to react.
     */
    public double adaptiveCoreChangeIntervalSeconds = 1.0;
    ...
    private boolean validate() {
        ...
        double validatedAdaptiveCoreChangeSeconds = sanitizeDouble(
                adaptiveCoreChangeIntervalSeconds,
                1.0,
                MIN_ADAPTIVE_CORE_CHANGE_SECONDS,
                MAX_ADAPTIVE_CORE_CHANGE_SECONDS
        );
        if (Double.compare(adaptiveCoreChangeIntervalSeconds, validatedAdaptiveCoreChangeSeconds) != 0) {
            logCorrection("adaptiveCoreChangeIntervalSeconds", adaptiveCoreChangeIntervalSeconds, validatedAdaptiveCoreChangeSeconds);
            adaptiveCoreChangeIntervalSeconds = validatedAdaptiveCoreChangeSeconds;
            changed = true;
        }
    ...
    private static boolean hasMissingConfigKeys(String json) {
        return !json.contains("\"showDebugInfo\"")
                ...
                || !json.contains("\"adaptiveCoreChangeIntervalSeconds\"")
